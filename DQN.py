...
def train():
    ...
    def dqn(n_episodes=100000, max_t=3000, eps_start=1.0, eps_end=0.01, eps_decay=0.995):
        ...
        for i_episode in range(1, n_episodes+1):
            ...
            poses, pose_of_hunter = env.reset(init_pose)
            ...
            for t in range(max_t):
                ...
                state = next_state
                score += reward
                # print(f"action:{real_action}, action:{action}, reward:{reward}")
                if done:
                    break
...
def evalutate() -> None:
    ...
    for i in range(3):
        ...
        for t in range(3000):
            ...
            next_state = np.concatenate((poses, pose_of_hunter)) # concatenate the environment state
            agent.step(state, action, reward, next_state, done)
            print(f"\raction:{real_action}, action:{action}, reward:{reward}", end="")
            state = next_state
            if done:
                break
...
if __name__ == "__main__":
    evalutate()
    # train()
    # A = np.random.uniform(-3, 3, size=(2,))
    # print(A)
